...
VARIANTS: dict[str, str] = {
    ...
    "watermark": "max-RGB + conservative ink threshold (cyan and gray logo suppressed)",
...
def chroma_mask(
    ...
    if dilate > 0:
        ...
        return cv2.dilate(mask, k).astype(np.uint8, copy=False)
    return mask
...
def build_variant(image: np.ndarray, name: str) -> np.ndarray:
    ...
    if name == "watermark":
        # The Fuyang overlay is partly cyan and partly neutral gray. max-RGB
        # removes the cyan pigment, but the gray logo remains around 170–190
        # while the printed genealogy strokes are substantially darker. Keep
        # only conservative ink pixels; unlike adaptive thresholding this does
        # not reinterpret the broad gray logo as locally dark foreground.
        _, out = cv2.threshold(ink_channel(bgr), 150, 255, cv2.THRESH_BINARY)
        return out
    if name == "lab_l":
    ...
    if name == "neutral":
        ...
        neutral_mask = chroma_mask(bgr) > 0
        ...
        out = base.copy()
        out[neutral_mask] = np.maximum(base[neutral_mask], bg[neutral_mask])
        return out

    if name == "inpaint":
        base = ink_channel(bgr)
        inpaint_mask = chroma_mask(bgr, dilate=5)
        return cv2.inpaint(base, inpaint_mask, 3, cv2.INPAINT_TELEA)

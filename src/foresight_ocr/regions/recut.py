...
from ..segmentation.entries import (
    anchored_phase_offset,
    entry_boundaries,
    resolve_comb,
)
from . import store
...
class CombInputs:
    ...
    corpus_pitch: float
    # Corpus-level phase of an entry boundary, as a fraction of the selected
    # pitch. The editor's slider is an adjustment on top of this verified base.
    base_phase_fraction: float = 0.0
    # Canonical x of the first interior right-edge boundary across the volume.
    # It disambiguates the two whitespace phases half an entry apart.
...
class CombPlan:
    ...
    page_index: int
    pitch: float
    phase_offset: float  # effective offset used to cut the page
    phase_adjustment: float  # reviewer adjustment relative to the base
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "base_phase_offset": self.base_phase_offset,
            "snap": self.snap,
...
def comb_inputs(
    ...
    if row is None:
        raise PageNotSegmentable(
            f"{document_id} p{page_index} has no layout; run `foresight-ocr layout` first"
        )
    ...
    if not template_path.exists():
        raise PageNotSegmentable(
            f"missing {template_path}; run `foresight-ocr layout` first"
        )
    ...
    pitch, text_left, text_right, used_corpus = resolve_comb(
        geometry,
        corpus_pitch,
        float(template["text_left"]),
        float(template["text_right"]),
    )
    ...
    return CombInputs(
        ...
        base_phase_fraction=float(template.get("entry_phase_fraction", 0.0)),
        phase_anchor_x=(
...
def plan_comb(
    ...
    *,
    ...
    boundary_overrides: Mapping[int, float] | None = None,
) -> CombPlan:
    ...
    configured_base_phase = use_pitch * inputs.base_phase_fraction
    base_phase_offset = anchored_phase_offset(
        left,
        right,
        use_pitch,
        inputs.gutters,
        configured_base_phase,
        inputs.phase_anchor_x,
        snap=snap,
    )
    effective_phase = base_phase_offset + phase_offset
    bounds, snapped = entry_boundaries(
        left,
        right,
        use_pitch,
        inputs.gutters,
        phase_offset=effective_phase,
        snap=snap,
    )
    manual = [False] * len(bounds)
    for raw_index, raw_x in (boundary_overrides or {}).items():
        index = int(raw_index)
        x = float(raw_x)
        if index < 0 or index >= len(bounds):
            raise PageNotSegmentable(
                f"manual boundary {index + 1} does not exist in this lattice"
            )
        if not math.isfinite(x) or x < 0 or x > inputs.page_width:
            raise PageNotSegmentable(
                f"manual boundary {index + 1} must stay on the page"
            )
        bounds[index] = x
        snapped[index] = False
        manual[index] = True
    ...
    if any(
        ...
        for right_edge, left_edge in zip(bounds, bounds[1:], strict=False)
    ):
        raise PageNotSegmentable("manual boundaries must remain in right-to-left order")
        ...
        for band in inputs.bands:
            for i in range(len(bounds) - 1):
                ...
                bbox = [
                    max(x0, 0.0),
                    band.top,
                    min(x1, float(inputs.page_width) or x1),
                    band.bottom,
                ]
                proposals.append(
                    Proposal(
...
def apply_comb(
    ...
    *,
    ...
    existing = [
        r
        for r in store.for_page(conn, document_id, page_index)
    ...
    if reocr:
        if on_progress:
            on_progress({"stage": "recutting", "completed": 0, "total": len(touched)})
        _read_page(
            conn,
            project,
            document_id,
    ...
    elif touched and project is not None:
        # Re-cutting and re-reading are separate choices in the review UI.
        # Even when the reviewer declines a model call, the changed geometry
        # still needs a current crop; otherwise /api/page falls back to a stale
        # crop box and the accepted lattice appears not to have applied.
        _cut_page(conn, project, touched, report, variant=variant)
    return report
...
def _cut_page(
    ...
    *,
    ...
    """Materialize current crops without asking an OCR backend to read them."""
    from .crops import CropUnavailable, ensure_crop

    before = conn.execute("SELECT COUNT(*) n FROM region_crops").fetchone()["n"]
    for region in store.get_many(conn, region_uids):
        try:
            ensure_crop(conn, project, region, variant=variant, context="tight")
        except CropUnavailable as exc:
            report.errors.append(str(exc))
    conn.commit()
    report.crops_cut = (
        conn.execute("SELECT COUNT(*) n FROM region_crops").fetchone()["n"] - before
    )
...
def _corrections_by_region(
    ...
    by_key = {(r.band_label, r.reading_order): r.region_uid for r in regions}

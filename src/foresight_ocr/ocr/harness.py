...
from foresight_ocr.ocr.base import OCRRequest, OCRResult, get_backend
from foresight_ocr.ocr.fields import parse_entry
from foresight_ocr.ocr.metrics import Pair, Score, rare_characters, score_pairs
...
def gold_score(
    ...
    return score_pairs(
        ...
        rare=rare
        if rare is not None
        else rare_characters([p.reference for p in pairs]),
    )
...
def load_outcomes(conn, document_id: str) -> list[RunOutcome]:
    ...
    rows = conn.execute(
        """SELECT m.backend AS backend, m.version AS model_version,
           ...
           WHERE sr.document_id = ? AND sr.role = 'entry'
           ORDER BY oc.id""",
        (document_id,),
    ).fetchall()

    grouped: dict[tuple[str, str, str, str], RunOutcome] = {}
...
def sequence_totals(seq: dict[str, Any]) -> tuple[float, float]:
    ...
    clean = (
        sum(b["clean_run_rate"] * max(b["parsed"] - 1, 0) for b in seq.values())
        / weights
    )
...
def store_results(conn, document_id: str, run_id: int, outcome: RunOutcome) -> int:
    ...
    cur = conn.execute(
        "INSERT INTO ocr_runs (run_id, model_id, input_variant, tag) VALUES (?,?,?,?)",
    ...
    for res in outcome.results:
        ...
        cand = conn.execute(
            ...
            (
                ...
                res.confidence,
                json.dumps(
                    {"raw": res.raw, "error": res.error, "latency_ms": res.latency_ms},
                    ensure_ascii=False,
                ),
            ),
        )

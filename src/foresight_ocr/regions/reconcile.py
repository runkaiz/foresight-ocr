...
def pair_up(
    ...
    for proposal in proposals:
        ...
        if candidates:
            # A reviewer can replace a retired machine box with a new adjusted
            # region at exactly the same geometry (a half-phase page repair does
            # this for columns beyond the identity-overlap threshold).  The
            # historical row has the lower id, so insertion order alone revives
            # it beside the live edit and leaves two identical columns.  Current
            # human-owned geometry is the authoritative identity; tombstones
            # only come back when no live region already answers the proposal.
            region = min(
                candidates,
                key=lambda r: (
                    r.deleted_at is not None,
                    r.state == RegionState.REJECTED,
                    not r.pinned,
    ...
    unmatched_proposals = [
        p for i, p in enumerate(remaining) if i not in taken_proposals
    ]
...
def reconcile_page(
    ...
    *,
    ...
    if not dry_run:
        for label in {r.band_label for r in existing} | {
            p.band_label for p in proposals
        }:
...
def _reconcile_band(
    ...
    for proposal, region, score in matched:
        ...
        digest = geometry_hash(document_id, page_index, proposal.bbox)
        if (
            region.geometry_hash == digest
            and region.deleted_at is None
            and region.role == proposal.role
            and region.entry_index == proposal.entry_index
        ):
            report.unchanged += 1
            continue

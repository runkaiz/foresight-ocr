...
from foresight_ocr.layout.frame import FrameFit, homography
...
def median_frame(passes: list[FramePass]) -> list[list[float]] | None:
    ...
    """
    good = [
        p for p in passes if p.fit.ok and p.fit.corners and not p.fit.inferred_edges
    ]
    if len(good) < 20:
        return None
...
def normalize_page(
    ...
    if not fit.ok:
        return PageNormalization(
            ...
            False,
            "failed",
            fit.reason or "frame fit failed",
            fit.skew_deg,
            fit.width,
            fit.height,
            float("inf"),
...
def warp_page(
    image: np.ndarray, forward: list[list[float]], space: CanonicalSpace
) -> np.ndarray:

...
from ..regions.crops import CropUnavailable, ensure_crop
    ...
    *,
    ...
    for region in regions:
        ...
        if not force:
            ...
            if row is not None:
                answers.append(
                    RegionAnswer(region.region_uid, row["transcription"], key, True)
                )
                continue
    ...
    if todo:
        ...
        answers.extend(
            _read(conn, document_id, engine, backend, options or {}, tag, variant,
                  context, todo)
        )
...
def _read(conn, document_id, engine, backend, options, tag, variant, context, todo):
    ...
    for region, crop in todo:
        ...
        conn.execute(
            ...
            (
                region.id, ocr_run_id, crop.crop_key, model, key,
...
def _open_run(conn, document_id, engine, backend, model, tag, variant) -> int:
    ...
    processing_run_id = conn.execute(
        ...
        (
            document_id, row["stage"], row["params_json"], row["params_hash"],
            None, row["compute_backend"], row["pipeline_version"], row["git_commit"],
            row["started_at"], _now(),
        ),
    ).lastrowid
    return int(

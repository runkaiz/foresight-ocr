...
from foresight_ocr.layout.lines import Line, detect_rules, merge_close
...
def find_candidates(
    ...
    sh, sw = small.shape[:2]
    h_lines = merge_close(
        detect_rules(small, "h", min_coverage=min_coverage), sw, tol=6
    )
    v_lines = merge_close(
        detect_rules(small, "v", min_coverage=min_coverage), sh, tol=6
    )
    f = float(downscale)
    return [_scale_line(ln, f) for ln in h_lines], [
        _scale_line(ln, f) for ln in v_lines
    ]
...
def _assemble(
    ...
    try:
        ...
        return FrameFit(
            [], 0.0, 0.0, 0.0, float("inf"), float("inf"), [], 4, False, str(exc)
        )
...
def detect_frame(
    ...
    if len(h_lines) < 2 or len(v_lines) < 2:
        ...
        fit = _assemble(
            h_lines[0], h_lines[-1], v_lines[0], v_lines[-1], h_lines, image.shape, []
        )
    ...
    if vpair is None:
        ...
        if vpair is None:
            return FrameFit(
                ...
                0,
                False,
                "no vertical rule can anchor the frame",
            )
    ...
    if hpair is None:
        ...
        if hpair is None:
            return FrameFit(
                ...
                0,
                False,
                "no horizontal rule can anchor the frame",
            )
        inferred.append(side)

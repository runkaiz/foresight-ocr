...
from familyocr.validation.numerals import format_numeral, parse_entry_id
    ...
    for band, items in by_band.items():
        ...
        for obs in items:
            ...
            if not parsed.ok or parsed.value is None:
                findings.append(Finding(
                    kind="unparsed", band_label=band, page_index=obs.page_index,
                    entry_index=obs.entry_index, observed=obs.text,
                    expected=None, detail=parsed.reason,
                ))
                continue
            if parsed.band is not None and parsed.band != band:
                findings.append(Finding(
                    kind="band_mismatch", band_label=band,
                    page_index=obs.page_index, entry_index=obs.entry_index,
                    observed=obs.text, expected=band,
        ...
        for i, (obs, value) in enumerate(values):
            if value in seen:
                prev = seen[value]
                findings.append(Finding(
                    kind="duplicate", band_label=band, page_index=obs.page_index,
                    entry_index=obs.entry_index, observed=obs.text,
                    expected=None,
            ...
            if step <= 0:
                findings.append(Finding(
                    kind="non_monotonic", band_label=band,
                    page_index=obs.page_index, entry_index=obs.entry_index,
                    observed=obs.text, expected=expected,
                    detail=f"{prev_value} -> {value}",
                ))
            else:
                findings.append(Finding(
                    kind="gap", band_label=band, page_index=obs.page_index,
                    entry_index=obs.entry_index, observed=obs.text,
                    expected=expected,

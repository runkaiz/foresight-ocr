...
from foresight_ocr.ocr.fields import ParsedEntry, parse_entry
...
def resolve_entry(
    text: str, geometric: str, charted: list[str], parent_of: dict[str, str | None]
) -> tuple[str, ParsedEntry]:
...
def _order_conflicts(
    people: dict[str, EntryRow], resolved: dict[str, str | None]
) -> list[GraphFinding]:
    ...
    for fkey, kids in siblings.items():
        ...
        for (e1, r1), (e2, r2) in zip(known, known[1:], strict=False):
            if r2 < r1:
                findings.append(
                    GraphFinding(
                        kind="order_reversed",
                        band_label=e2.geometric_label,
                        page_index=e2.page_index,
                        entry_index=e2.entry_index,

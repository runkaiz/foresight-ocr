...
def draw_frame_overlay(
    ...
    if corners:
        ...
        for i, p in enumerate(pts):
            ...
            cv2.putText(
                canvas, "TL TR BR BL".split()[i], tuple(p + np.array([8, -8])),
    ...
    if caption:
        for i, line in enumerate(caption.split("\n")):
            cv2.putText(
                canvas, line, (8, 18 + 16 * i), cv2.FONT_HERSHEY_SIMPLEX,
...
def draw_grid_overlay(
    ...
    for y in band_edges:
        cv2.line(canvas, (0, int(y * scale)), (w, int(y * scale)), GREEN, 2, cv2.LINE_AA)
    for x in column_edges:
        cv2.line(canvas, (int(x * scale), 0), (int(x * scale), h), AMBER, 1, cv2.LINE_AA)

    if caption:
        cv2.putText(canvas, caption, (8, 18), cv2.FONT_HERSHEY_SIMPLEX,
                    0.45, (0, 0, 0), 3, cv2.LINE_AA)
        cv2.putText(canvas, caption, (8, 18), cv2.FONT_HERSHEY_SIMPLEX,
                    0.45, GREEN, 1, cv2.LINE_AA)
...
def contact_sheet(paths: list[Path], out_path: Path, cols: int = 5,
                  cell: tuple[int, int] = (260, 400)) -> Path:

...
def model_key(
    backend: str, model_version: str, options: dict[str, Any] | None = None
) -> str:
...
def cache_key(crop: str, model: str, tag: str = "") -> str:
    ...
    return hashlib.sha256(f"{crop}|{model}|{tag or ''}".encode("utf-8")).hexdigest()[
        :24
    ]
...
def pending(
    ...
    *,
    ...
    # Every entry in ``where`` is a local SQL literal; values live in ``params``.
    rows = conn.execute(
        f"""

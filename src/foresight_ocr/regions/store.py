    ...
    *,
    ...
    conn.execute(
        ...
        (
            ...
            page_index,
            json.dumps(box),
            geometry_hash(document_id, page_index, box),
            transform_id,
...
def reposition(
    ...
    *,
    ...
    conn.execute(
        ...
        "reading_order = CASE WHEN reading_order_locked = 0 THEN COALESCE(?, reading_order) "
        "ELSE reading_order END, role = COALESCE(?, role), "
...
def retire(
    conn: sqlite3.Connection, region: Region, *, run_id: int | None = None
) -> None:
